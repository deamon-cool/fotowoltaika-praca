...
import {
  selectAdvertiserID, selectEntryKey
...
import classes from './EditAdvertiserAd.module.css';
import ToastBar from '../../components/ToastBar/ToastBar';
import Button from '../../components/Button/Button';

function EditAdvertiserAd(props) {
  ...
  const sendAdHandler = async (e) => {
    ...
    if (inputChanged) {
      ...
      const url = modifyFetchUrl(config.EDITADVERTISERAD_EDIT_INPUTS_FETCH);
      const init = {
        method: 'POST',
        headers: {
    ...
    if (editorStateChanged) {
      ...
      const url = modifyFetchUrl(config.EDITADVERTISERAD_EDIT_DESCRIPTION_FETCH);
      const init = {
        method: 'POST',
        headers: {
          "Content-Type": "application/json",
  ...
  const editImageOnServer = async (command, serverInfos, serverWarnings) => {
    ...
    const url = modifyFetchUrl(config.EDITADVERTISERAD_EDIT_IMAGE_FETCH + idKeyParam + '/' + command);
    const init = {
      method: 'POST',
      headers: {
        advertiserID: advertiserID,
  ...
  return (
    <div className={classes.EditAdvertiserAd}>
      <ToastBar
        info={info}
        warning={warning}
...
export default withRouter(EditAdvertiserAd);